...
%分集2
Fd0=100.3;              %Doppler频偏，以Hz为单位
tau0=[0,1/fc,2/fc];          %多径延时，以s为单位
pdb0=[0,-4,-6];          %各径功率，以dB位单位
h0=rayleighchan(1/fs,Fd,tau0,pdb0);
y2=filter(h0,bpsk);
h2=zeros(1,length(y2));

%分集3
Fd3=1.11;              %Doppler频偏，以Hz为单位
tau3=[0,3/fc,4/fc];          %多径延时，以s为单位
pdb3=[0,-8,-9];          %各径功率，以dB位单位
h3=rayleighchan(1/fs,Fd,tau3,pdb3);
y3=filter(h3,bpsk);
h3=zeros(1,length(y3));

%分集4
Fd4=243.5;              %Doppler频偏，以Hz为单位
tau4=[0,5/fc,1/fc];          %多径延时，以s为单位
pdb4=[0,-8,-3];          %各径功率，以dB位单位
...
y22=y2.*ct.*h2;%瑞利信号解调2
y33=y3.*ct.*h3;%瑞利信号解调3
y44=y4.*ct.*h4;%瑞利信号解调3
avr=mean(abs(y1).^2);
db=-10*log10(avr);
%过LPF带宽为5Rs
[f0,sf0]=T2F(t,y11);
...
plot(t,dpsk0);
xlabel('t');
title('2psk解调信号之后过LPF');

subplot(3,2,3);
plot(t,y11);
xlabel('t');
title('过瑞利信道1之后的信号r1(t)*载波*估计信号');


subplot(3,2,4);
plot(t,ruili);
xlabel('t');
title('r1(t)*载波*估计信号之后过LPF');

subplot(3,2,5);
plot(abs(fft(dpsk0)));
grid on;
title('2psk解调信号过LPF后频谱');xlabel('f');

subplot(3,2,6);
plot(abs(fft(ruili)));
grid on;
title('过瑞利信道1之后的信号r1(t)*载波*估计信号频谱');xlabel('f');
...
plot(t,ruili1);
xlabel('t');
title('r2(t)*载波*估计信号之后过LPF');
%}
ruili0=zeros(1,length(ruili1));
for i=1:length(ruili)%选择合并
